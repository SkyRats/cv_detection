...
while(True):
    ...
    corrigido = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    blur1 = cv2.GaussianBlur(corrigido, (9,9), 0)
    ...
    for cnt in cnts:
        ...
        #if len(approx) == 3:
            #cv2.putText(frame, "Triangulo", (x,y), font, 1, (0))
            #cv2.drawContours(frame, [approx], 0, (0), 2)
        if len(approx) == 12:
            ...
            cv2.imshow("smaller", small_img)
            result = small_img.dot(kernel)
            #############################################################################

            #if small_img[0][0] and small_img[1][0] 
            ...
            #      print(v)
            #      print("\n")
            #     cv2.putText(frame, str(i), (v[0][0], v[0][1]), font, 1, (0, 255, 0))
            #     i += 1
